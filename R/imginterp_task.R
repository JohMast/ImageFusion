...
#' @param layers A list of numeric (see below at \code{<num-list>}). Specifies the channels or layers, that will be read. Hereby a 0 means the first channel. Should be either matching \code{filenames} in length and order or be of length 1, in which case the selection will be applied to all images.
#' @param verbose (Optional) Output intermediate progress reports? Default is "true".
#' @param interp_invalid 
#' @param interp_ranges 
#' @param no_interp_ranges 
#' @param stats_filename 
#' @param print_stats 
#' @param out_prefix 
#' @param out_postfix 
#' @param output_pixelstate 
#' @param use_nodata 
#' @param prioritize_invalid 
#' @param out_pixelstate_prefix 
#' @param out_pixelstate_postfix 
...
#' @param ql_interp_ranges (Optional, only used if any \code{ql_filenames} were provided.) A list of character (see below at \code{<range-list>}). Specifies the ranges of the shifted value that should mark the location for interpolation. Should be either matching \code{ql_filenames} in length and order or be of length 1, in which case the ranges will be applied to all ql_images.
#' @param ql_non_interp_ranges (Optional, only used if any \code{ql_filenames} were provided.) A list of character (see below at \code{<range-list>}). Specifies the ranges of the shifted value that should mark the location for interpolation. Should be either matching \code{ql_filenames} in length and order or be of length 1, in which case the ranges will be applied to all ql_images.
